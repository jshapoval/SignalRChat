...
//                .Include(x => x.Participants).ThenInclude(x => x.Sender).ThenInclude(x => x.Avatar)
//                .Where(d => d.Participants.Any(p => p.AppUserId == userId))
//                .Skip(offset)
//                .Take(count)
//                .ToListAsync();//.Where(x => x.Status != DialogStatus.Canceled)
//        }
