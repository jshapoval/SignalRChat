...
namespace ChatServerSignalRWithIdentity.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var friendRelationshipList =
                currentUser.Relationships.Where(i => i.Status == RelationshipStatus.Friend);
            string friendId;
            //var friends = new List<AppUser>();
            var dialogsWithFriends = new List<Dialog>();
            var sortedDialogs = new List<Dialog>();
            var sortedMessages = new List<Message>();

            foreach (var friend in friendRelationshipList)
            {
                friendId = currentUser.Id == friend.BigUserId ? friend.SmallUserId : friend.BigUserId;

                var friendByUsers = users.Find(i => i.Id == friendId);
                //friends.Add(friendByUsers);
                var dialogForList = dialogs.Find(n =>
                    (n.Participants.First().AppUserName == currentUser.UserName) &&
                    (n.Participants.Last().AppUserName == friendByUsers.UserName) ||
                    (n.Participants.First().AppUserName == friendByUsers.UserName) &&
                    (n.Participants.Last().AppUserName == currentUser.UserName));

                dialogsWithFriends.Add(dialogForList);
            }

            if (dialogsWithFriends.Count != 0)
            {
                sortedDialogs = dialogsWithFriends;
            }
            ...
            var response = new ChatModel
            {
                ...
                MessagesList = _mapper.Map<List<Message>>(sortedMessages),
                DialogsWithFriendsList = _mapper.Map<List<Dialog>>(sortedDialogs),
            };
